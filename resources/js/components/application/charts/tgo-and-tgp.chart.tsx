...
export default function TgoAndTgpChart({ chartData, total }: Readonly<Props>) {
    const { __ } = lang();
    ...
    return (
        <Card className="mx-auto max-w-3xl">
            <CardHeader>
                <CardTitle>{__('tgo_and_tgp_pages.index.chart.title')}</CardTitle>
                <CardDescription>{__('tgo_and_tgp_pages.index.chart.description')}</CardDescription>
            </CardHeader>
            <CardContent>
                <ChartContainer config={chartConfig}>
                    ...
                    >
                        ...
                        <XAxis
                            ...
                            axisLine={false}
                            tickMargin={8}
                            tickFormatter={(value) => format(String(value), 'PPP')}
                        ...
                        {Object.entries(chartConfig).map(([key]) => (
                            ...
                            >
                                <LabelList
                                    ...
                                    fontSize={12}
                                    formatter={(value) => `${value} ${__('tgo_and_tgp_pages.shared.unit')}`}
                                />
                            </Line>
                        ))}
                    </LineChart>
            ...
            <CardFooter className="border-t">
                <p className="text-muted-foreground text-sm">
                    {__('tgo_and_tgp_pages.index.chart.footer_total_label')} <strong>{total}</strong>
                </p>
            </CardFooter>
        </Card>
    );
