...
export function DataTableDateFilter<TData>({ column, title, multiple }: Readonly<DataTableDateFilterProps<TData>>) {
    ...
    const formatDateRange = useCallback((range: DateRange) => {
        ...
        if (range.from && range.to) {
            return `${format(range.from, 'MMMM d, yyyy')} - ${format(range.to, 'MMMM d, yyyy')}`;
        }

        return format(String(range.from ?? range.to), 'MMMM d, yyyy');
    }, []);
    ...
    const label = useMemo(() => {
        if (multiple) {
            ...
            return (
                <span className="flex items-center gap-2">
                    <span>{title}</span>
                    <Activity mode={hasSelectedDates ? 'visible' : 'hidden'}>
        ...
        const dateText = hasSelectedDate ? format(selectedDate, 'MMMM d, yyyy') : 'Select date';

        return (
            <span className="flex items-center gap-2">
                <span>{title}</span>
                <Activity mode={hasSelectedDate ? 'visible' : 'hidden'}>
                    <Separator orientation="vertical" className="mx-0.5 data-[orientation=vertical]:h-4" />
                    <span>{dateText}</span>
                </Activity>
            </span>
