...
- Nested groups are passed to this package with dot notation, e.g. `syncLangFiles('admin.users')` reads `lang/{locale}/admin/users.php` and nests the result so the frontend resolves it with `__('admin.users.name')`.
- Laravel itself addresses a translation file inside a nested directory with `/`, so the same source translation is called on the backend with `__('admin/users.name')`, not `__('admin.users.name')`.
